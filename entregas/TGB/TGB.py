...
def apply_filter(filter, image):
    ...
    if filter == 4:
        for i in range(image.shape[0]):
            for j in range(image.shape[1]):
                image.itemset((i,j,0),image.item(i,j,0)^255)
                image.itemset((i,j,1),image.item(i,j,1)^255)
                image.itemset((i,j,2),image.item(i,j,2)^255)

    if filter == 5:
        image = cv.cvtColor(image,cv.COLOR_BGR2GRAY)
        for i in range(image.shape[0]):
            for j in range(image.shape[1]):
                if image.item(i,j) < 120:
                    image.itemset((i,j),0)
                else:
    ...
    if filter == 7:
        for i in range(image.shape[0]):
            for j in range(image.shape[1]):
                avg = image.item(i,j,0) * 0.333 + image.item(i,j,1) * 0.333 + image.item(i,j,2) * 0.3333
                image.itemset((i,j,0),avg)
                image.itemset((i,j,1),avg)
                image.itemset((i,j,2),avg)
...
sample_image_path = os.path.join(work_dir, 'dog.png')
